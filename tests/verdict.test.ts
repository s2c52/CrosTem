...
describe('computeVerdict (política conservadora)', () => {
  it('verde: CodeWeavers Runs Great sin contradicciones', () => {
    expect(computeVerdict(cwGreat, null, null)).toBe('green');
  });

  it('verde: AGW playable aunque no haya datos de CodeWeavers', () => {
    expect(computeVerdict(null, agw('playable'), null)).toBe('green');
  });

  it('verde: buenas señales y anticheat Supported', () => {
    expect(computeVerdict(cwGreat, agw('perfect'), ac('Supported'))).toBe('green');
  });

  it('amarillo: fuentes en desacuerdo (CW bien, AGW unplayable)', () => {
    expect(computeVerdict(cwGreat, agw('unplayable'), null)).toBe('yellow');
  });

  it('amarillo: señal intermedia (3 estrellas sin estado)', () => {
    expect(computeVerdict({ stars: 3 }, null, null)).toBe('yellow');
  });

  it('amarillo: buena señal pero anticheat Planned (incierto)', () => {
    expect(computeVerdict(cwGreat, null, ac('Planned'))).toBe('yellow');
  });

  it('rojo: anticheat Denied aplasta cualquier señal buena', () => {
    expect(computeVerdict(cwGreat, agw('perfect'), ac('Denied'))).toBe('red');
  });

  it('rojo: anticheat Broken también bloquea', () => {
    expect(computeVerdict(cwGreat, null, ac('Broken'))).toBe('red');
  });

  it('rojo: solo señales malas (CW will not run)', () => {
    expect(computeVerdict(cwBad, null, null)).toBe('red');
  });

  it("rojo: AGW doesn't work sin señal buena", () => {
    expect(computeVerdict(null, agw("doesn't work"), null)).toBe('red');
  });

  it('unknown: sin ningún dato', () => {
    expect(computeVerdict(null, null, null)).toBe('unknown');
  });

  it('unknown: AGW na no cuenta como señal', () => {
    expect(computeVerdict(null, agw('na'), null)).toBe('unknown');
  });

  it('rojo: sin datos de compatibilidad pero anticheat bloqueado', () => {
    expect(computeVerdict(null, null, ac('Denied'))).toBe('red');
  });

  it('anticheat Running no penaliza señales buenas', () => {
    expect(computeVerdict(cwGreat, null, ac('Running'))).toBe('green');
  });
});
